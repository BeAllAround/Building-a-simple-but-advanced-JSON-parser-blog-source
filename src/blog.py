...
import sys
import utils
from parser import Parser # our library for lexical analysis/tokenization
...
def expr(token: Parser, get: bool, scope: dict, flags: utils.Map):
...
def term(token: Parser, get: bool, scope: dict, flags: utils.Map):
...
def prim(token: Parser, get: bool, scope: dict, flags: utils.Map):
    if get:
        token.next_token()
    skip_space(token)
    if token == '(':
...
def chain_main(token: Parser, value_ptr: list, scope: dict):
    while True:
...
def parse_obj_and_chain(token: Parser, get: bool, scope: dict, flags: utils.Map):
    value = parse_obj(token, get, scope, flags)
    value_ptr = [value]
    chain_main(token, value_ptr, scope)
    return value_ptr[0]

def parse_obj(token: Parser, get: bool, scope: dict, flags: utils.Map):
    if get:
        token.next_token()
